import { getPosition, createLanguageServiceWithVolarTSPlugin, extractLocation } from "./util.ts";
import path from "node:path";

const projectRoot = process.cwd();
const targetFile = path.join(projectRoot, "src/index.ts");

const languageService = createLanguageServiceWithVolarTSPlugin(projectRoot);
...
process.exit(0); // Force exit because the process does not terminate for some reason
